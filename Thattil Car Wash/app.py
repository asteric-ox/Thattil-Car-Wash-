...
def approve_pickup(booking_id):
    try:
        ...
        try:
            ...
            msg = MIMEMultipart()
            msg['From'] = f"D2 Admin <{sender_email}>"
            msg['To'] = staff_email
            msg['Subject'] = f"✅ PICKUP APPROVED - {booking['customer_name']}"
            body = f"""
            <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;padding:20px;border:1px solid #eee;border-radius:10px;">
                <h2 style="color:#06b6d4;">Pickup Approved</h2>
        ...
        return jsonify({"success": True, "message": f"Pickup approved. Phone sent to staff."})
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500
    ...
    try:
        ...
        if new_status == 'Confirmed':
            try:
                ...
                print(f"Staff email error on confirm: {mail_err}")
...
def get_admin_analytics():
    try:
        ...
        # Revenue - Corrected field to 'amount'
        try:
...
def verify_booking():
    ...
    if is_pickup_qr:
        # --- Pickup QR scan → set status to Confirmed ---
        cursor.execute(
        ...
        if booking.get('user_id'):
            cursor.execute(
                "INSERT INTO notifications (user_id, message) VALUES (%s, %s)",
                (booking['user_id'], "✅ Your pickup has been confirmed via QR scan! Staff will arrive shortly.")
            )
        conn.commit()
        cursor.close()
        conn.close()
        return jsonify({
            "success": True,
            "message": f"Pickup #{str(booking_id).zfill(4)} QR verified! Status set to Confirmed.",
